...
4. If either role assignment is missing, do not ask for launch details. Tell the user to go back to `/role` and complete role assignment first.
...
8. Explain that the loop is now automatic and the enrolled developer/debugger threads should not be manually driven until the loop completes, blocks, fails, or is landed with `/devise-land`.
